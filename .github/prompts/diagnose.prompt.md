---
mode: agent
description: Deep bug diagnosis and root cause analysis when simple fixes have failed
tools:
...
- **Required Reconnaissance Steps:**
  ...
  7. **Anomaly-Specific Scan:** Review logs, error messages, and recent changes related to the bug.
...
- **Actions:**
  ...
  4. **Minimize the Case:** Strip down the test case to the absolute minimum required to reproduce the issue.
...
- **Evidence-Gathering Protocol:**
  ...
  4. **Iterate Until Root Cause:** Continue hypothesis-testing until the root cause is confirmed with irrefutable evidence.
...
- **RCA Best Practices:** Use debugging tools, logs, code inspection, and controlled experiments. Document each hypothesis and outcome.

## Phase 3: Remediation

- **Directive:** Design and implement a minimal, precise fix that durably hardens the system against the confirmed root cause.
- **Remediation Steps:**
  1. **Design the Fix:** Propose the minimal change required to address the root cause.
  2. **Implement Incrementally:** Apply the fix in small, testable increments.
  3. **Justify the Approach:** Explain why this fix prevents the root cause without over-engineering.
- **Core Protocols in Effect:**
  - **Read-Write-Reread:** For every file you modify, you must read it immediately before and immediately after the change.
  - **Command Execution Canon:** All shell commands must use the mandated safety wrapper (e.g., timeout, non-interactive flags, fail-fast).
...
- **Verification Steps:**
  ...
  4. **End-to-End Testing:** Perform end-to-end tests on affected workflows.
...
- **Audit Protocol:**
  ...
  3. **Empirical Re-Check:** Re-run the original failing test and quality gates with fresh evidence.
...
- Root cause must be evidenced-based; no speculative fixes.
